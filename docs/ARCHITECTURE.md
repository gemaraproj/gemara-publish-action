...
- A vendored Go CLI (`tools/publish/main.go`) that wires `bundle.Assemble` + `bundle.Pack` +
...
|---------|-----------|-------|
| Publish source | go-gemara SDK via vendored CLI | Caller provides root Gemara YAML; CLI runs Assemble + Pack + oras.Copy. |
...
The vendored CLI delegates all bundle semantics (assembly, packing, manifest construction) to
