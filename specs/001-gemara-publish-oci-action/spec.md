...
**Test coverage:** `.github/workflows/ci.yml` builds `tools/publish/`, runs a dry-run assemble/pack
against `testdata/minimal-catalog.yaml`, and validates the publisher compiled successfully.
...
1. **Set up Go** and build the vendored publisher CLI from `tools/publish/`.
...
**In scope:** ORAS install pin, registry auth, go-gemara SDK publish via vendored CLI, sign/verify
...
The Action SHALL build the vendored Go CLI (`tools/publish/`), invoke it with the caller's `file`,
